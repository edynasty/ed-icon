...
// 模板：图标索引文件
const indexTemplate = `// 自动生成的品牌 Vue 图标组件导出文件
...
function generateVueComponents() {
  const iconsDir = path.resolve(__dirname, "../../icons-brand-svg/src/icons");
  ...
  iconFiles.forEach((file) => {
    ...
    try {
      ...
      console.log(`Generated brand icon: ${iconName}.vue`);
    } catch (error) {
  ...
  console.log(`\nGenerated ${icons.length} brand Vue icon components`);
  console.log("Brand icons index file updated");
}
...
try {
  ...
  console.error("Error generating brand Vue components:", error);
  process.exit(1);
}
