...
`;
  ...
  console.log(`生成技术图标文件: ${iconName}.ts`);
...
function generateAllIcons() {
  ...
  // 生成导出文件
  const exportContent = `// 自动生成的技术图标导出文件
${iconData
...
// 导出技术图标名称列表
export const TechIconNames = [
${iconData.map((icon) => `  '${icon.fileName}',`).join("\n")}
] as const

export type TechIconName = typeof TechIconNames[number]
`;

  const exportFilePath = path.join(iconsDir, "index.ts");
  fs.writeFileSync(exportFilePath, exportContent);

  console.log("生成技术图标导出文件: index.ts");
  console.log(`总共生成了 ${iconData.length} 个技术图标定义文件`);
}
