...
// 模板：图标索引文件
const indexTemplate = `// 自动生成的 Vue 图标组件导出文件
...
function generateVueComponents() {
  const iconsDir = path.resolve(__dirname, "../../icons-svg/src/icons");
  ...
  console.log(`\nGenerated ${icons.length} Vue icon components`);
...
try {
  ...
  console.error("Error generating Vue components:", error);
  process.exit(1);
}
